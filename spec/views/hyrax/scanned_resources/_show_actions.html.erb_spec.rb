...
RSpec.describe "hyrax/scanned_resources/_show_actions.html.erb" do
  let(:resource) do
  ...
  before do
    ...
    render partial: "hyrax/scanned_resources/show_actions", locals: { collector: collector, editor: editor }
  end
  it "renders a reorder link" do
    expect(rendered).to have_link I18n.t('file_manager.link_text'), href: file_manager_hyrax_scanned_resource_path(id: resource.id)
  end
  it "renders an Edit Structure link" do
    expect(rendered).to have_link "Edit Structure", href: structure_hyrax_scanned_resource_path(id: resource.id)
  end
  ...
  context "when there's a parent presenter" do
    ...
    it "renders a link to the contextual file manager" do
      expect(rendered).to have_link "File Manager", href: file_manager_hyrax_parent_scanned_resource_path(id: resource.id, parent_id: parent_presenter.id)
    end
  end
end
