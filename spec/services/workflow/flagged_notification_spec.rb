...
RSpec.describe Workflow::FlaggedNotification do
  ...
  describe ".send_notification" do
    it 'sends a message to all users' do
      ...
      expect(ActionMailer::Base.deliveries.first.subject).to eq "[plum] Scanned Resource #{work.id}: Flagged"
      expect(ActionMailer::Base.deliveries.first.to_s)
        .to include("The following Scanned Resource has been flagged by #{approver.user_key}:")
    end
  end
end
