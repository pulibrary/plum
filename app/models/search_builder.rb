class SearchBuilder < CurationConcerns::SearchBuilder
  ...
  def join_from_parent(solr_params)
    ...
    solr_params[:q] = "({!join from=ordered_by_ssim to=id}{!dismax}#{solr_params[:q]}) OR ({!dismax}#{solr_params[:q]}) OR ({!join from=generic_work_ids_ssim to=id}{!dismax}#{solr_params[:q]})"
  end
