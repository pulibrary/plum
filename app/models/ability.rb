class Ability
  ...
  def image_editor_permissions
    can [:read, :create, :modify, :update, :publish], hyrax
    ...
    cannot [:complete], hyrax

    # only allow deleting for own objects, without ARKs
    can [:destroy], FileSet, depositor: current_user.uid
    can [:destroy], hyrax, depositor: current_user.uid
    cannot [:destroy], hyrax do |obj|
      !obj.identifier.nil?
    end
  end

  def editor_permissions
    can [:read, :modify, :update], hyrax
    can [:file_manager, :save_structure], ScannedResource
    can [:file_manager, :save_structure], MultiVolumeWork
  ...
  def curation_concern_read_permissions
    cannot [:read], hyrax do |curation_concern|
      !readable_concern?(curation_concern)
    end
    can :pdf, (hyrax + [ScannedResourceShowPresenter]) do |curation_concern|
      ["color", "gray"].include?(Array(curation_concern.pdf_type).first)
    end
    can :color_pdf, (hyrax + [ScannedResourceShowPresenter]) do |curation_concern|
